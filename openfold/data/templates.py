...
def _get_pdb_id_and_chain(hit: parsers.TemplateHit) -> Tuple[str, str]:
    ...
    # PDB ID: 4 letters. Chain ID: one+ alphanumeric letters or "." if unknown.
...
def _find_template_in_pdb(
    ...
    This method tries the three following things in order:
        one. Tries if there is an exact match in both the chain ID and the sequence.
...
def _realign_pdb_template_to_query(
    ...
    if not new_template_sequence:
        if len(mmcif_object.chain_to_seqres) == 1:
            logging.info(
                "Could not find %s in %s, but there is only one sequence, so "
                "using that one.",
                template_chain_id,
...
def _build_query_to_hit_index_mapping(
    ...
    # Index of -one used for gap characters. Subtract the min index ignoring gaps.
