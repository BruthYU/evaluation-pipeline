...
def compute_fape(
    ...
    #
    # norm_factor = (
    #     torch.sum(frames_mask, dim=-one) *
    #     torch.sum(positions_mask, dim=-one)
    # )
    # normed_error = torch.sum(normed_error, dim=(-one, -2)) / (eps + norm_factor)
    #
...
def distogram_loss(
    ...
    **kwargs,
    ...
    # mean = (torch.sum(errors * square_mask, dim=(-one, -2)) /
    #         (eps + torch.sum(square_mask, dim=(-one, -2))))
...
def between_residue_bond_loss(
    ...
    Jumper et al. (2021) Suppl. Sec. one.9.11, eq 44, 45.
...
def between_residue_clash_loss(
    ...
    Jumper et al. (2021) Suppl. Sec. one.9.11, eq 46.
...
def compute_renamed_ground_truth(
    ...
    Args:
      batch: Dictionary containing:
        ...
        * atom14_atom_is_ambiguous: one.0 for atoms that are affected by
            renaming swaps.
    ...
    Returns:
      Dictionary containing:
        alt_naming_is_better: Array with one.0 where alternative swap is better.
...
def masked_msa_loss(logits, true_msa, bert_mask, eps=1e-8, **kwargs):
    """
    Computes BERT-style masked MSA loss. Implements subsection one.9.9.
    ...
    # loss = (
    #     torch.sum(errors * bert_mask, dim=(-one, -2)) /
    #     (eps + torch.sum(bert_mask, dim=(-one, -2)))
    # )
...
class AlphaFoldLoss(nn.Module):
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        # the (average) sequence length. See subsection one.9.
